...
function musicAsync(callback) {
  ...
  fs.mkdir(targetDir, { recursive: true }, (err) => {
    ...
    music.forEach(song => {
      const laguFolder = path.join(targetDir, song.fileName)
      
      fs.mkdir(laguFolder, { recursive: true }, (err) => {
        if (err && err.code !== 'EEXIST') {
          return callback(err)
        }

        const filePath = path.join(laguFolder, song.fileName)
